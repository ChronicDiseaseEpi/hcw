...
new_data_hh <- expand_grid(age10 = (0:85)/10, 
                           male = 0:1, 
                           role = c("npf", "pf_any", "undetermined"),
                           como = c("0", "1", "2pls"),
                           hosptlsd = 1L, stratum = "a", time = 31*3)
MakePredData <- function(mymod, new_data = new_data_hh){
  ...
  bh <- bh %>% 
    ...
    tail(1) %>% 
    pull(hazard)
  # Multiply the baseline cumulative hazard by the hazard ratio to obtain the cumualtive hazard for the given covariate levels
